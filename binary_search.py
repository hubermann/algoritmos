...
def binary_search(array, item):
    ...
    Returns:
        Índice del item si se encuentra, None si no existe
    """
    low = 0
    high = len(array) - 1
    ...
    while low <= high:
        ...
        mid = (low + high) // 2
        print(f"Paso 1 - Calculamos mid: ({low} + {high}) // 2 = {mid}")
        
        # PASO 2: Obtenemos el valor que está en esa posición
        guess = array[mid]
        print(f"Paso 2 - El valor en posición {mid} es: {guess}")
        
        # PASO 3: Comparamos con el item que buscamos
        if guess == item:
            print(f"Paso 3 - ¡Encontrado! {guess} == {item}")
            return mid
        elif guess > item:
            # PASO 4a: Si guess es mayor, buscamos en la mitad izquierda
            print(f"Paso 3 - {guess} > {item}, entonces buscamos en la izquierda")
            high = mid - 1
            print(f"Paso 4a - Nuevo high = {high}")
        else:
            # PASO 4b: Si guess es menor, buscamos en la mitad derecha
            print(f"Paso 3 - {guess} < {item}, entonces buscamos en la derecha")
            low = mid + 1
            print(f"Paso 4b - Nuevo low = {low}")
...
print(f"Buscando {target} en el array ordenado:")
print(f"Resultado: {binary_search(sorted_array, target)}")